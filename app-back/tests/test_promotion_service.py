...
class TestSnapshotTypes:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_snapshot_captures_workflow_metadata(self):
        """Snapshot should capture workflow JSON and minimal metadata (workflow_id, environment_id, promotion_id, created_at)."""
        with patch("app.services.promotion_service.db_service") as mock_db:
            mock_db.get_environment = AsyncMock(return_value={
                "id": "env-1",
                "n8n_type": "development",
            ...
            with patch("app.services.promotion_service.ProviderRegistry") as mock_registry:
                ...
                mock_adapter.get_workflow = AsyncMock(side_effect=[
                ...
                with patch("app.services.promotion_service.GitHubService") as mock_github:
                    ...
                    mock_github_instance.is_configured.return_value = True
                    mock_github_instance.sync_workflow_to_github = AsyncMock(return_value=None)
                    mock_github_instance.repo.get_commits.return_value = [MagicMock(sha="commit-abc123")]
                    mock_github_instance.branch = "main"
                    mock_github_instance._sanitize_foldername = MagicMock(return_value="development")
                    mock_github.return_value = mock_github_instance
                    ...
                    with patch("app.services.promotion_service.notification_service") as mock_notify:
                        ...
                        snapshot_id, commit_sha = await service.create_snapshot(
                            ...
                            environment_id="env-1",
                            reason="Pre-promotion backup",
                            metadata={"type": "pre_promotion", "promotion_id": "promo-123"}
                        )

                        # Verify the snapshot was created with all required metadata
                        call_args = mock_db.create_snapshot.call_args
                        snapshot_data = call_args[0][0]

                        # Check snapshot-level fields
                        assert snapshot_data["environment_id"] == "env-1"
                        assert snapshot_data["tenant_id"] == "tenant-1"
                        assert snapshot_data["git_commit_sha"] == "commit-abc123"
                        assert snapshot_data["type"] == "pre_promotion"

                        # Check metadata_json contains required fields
                        metadata_json = snapshot_data["metadata_json"]
                        assert metadata_json["promotion_id"] == "promo-123"
                        assert metadata_json["workflows_count"] == 2
                        assert "workflows" in metadata_json

                        # Verify workflow metadata includes workflow_id, name, and active state
                        workflows = metadata_json["workflows"]
                        assert len(workflows) == 2
                        assert workflows[0]["workflow_id"] == "wf-1"
                        assert workflows[0]["workflow_name"] == "Workflow 1"
                        assert workflows[0]["active"] is True
                        assert workflows[1]["workflow_id"] == "wf-2"
                        assert workflows[1]["workflow_name"] == "Workflow 2"
                        assert workflows[1]["active"] is False

                        # Verify created_at will be auto-generated by database (not in our data)
                        # This is handled by the database timestamp default
