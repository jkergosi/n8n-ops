...
class TestWorkflowsAPIGet:
    ...
    @pytest.mark.api
    def test_get_workflow_by_id_success(self, client: TestClient, mock_workflows, auth_headers):
        """GET /workflows/{id} should return specific workflow."""
        ...
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            ...
            with patch("app.api.endpoints.workflows.ProviderRegistry") as mock_registry:
                ...
                mock_adapter.get_workflow = AsyncMock(return_value=workflow.get("workflow_data", {}))
    ...
    def test_get_workflow_not_found(self, client: TestClient, auth_headers):
        """GET /workflows/{id} should return 404 for non-existent workflow."""
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            mock_db.get_environment = AsyncMock(return_value={
                "id": "env-1",
                "n8n_base_url": "https://n8n.example.com",
...
class TestWorkflowsAPIUpdate:
    ...
    @pytest.mark.api
    def test_update_workflow_success(self, client: TestClient, mock_workflows, auth_headers):
        """PUT /workflows/{id} should update workflow."""
        workflow = mock_workflows[0]
        update_data = {
        ...
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            ...
            with patch("app.api.endpoints.workflows.ProviderRegistry") as mock_registry:
                mock_adapter = MagicMock()
                mock_adapter.update_workflow = AsyncMock(return_value={
                    "id": workflow["n8n_workflow_id"],
                    "name": "Updated Workflow Name",
                    "nodes": [],
                    "connections": {},
                ...
                mock_db.update_workflow = AsyncMock(return_value=None)
...
class TestWorkflowsAPITags:
    """Tests for workflow tags endpoint."""

    @pytest.mark.api
    def test_update_workflow_tags_success(self, client: TestClient, mock_workflows, auth_headers):
        """PUT /workflows/{id}/tags should update workflow tags."""
        workflow = mock_workflows[0]
        ...
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            ...
            with patch("app.api.endpoints.workflows.ProviderRegistry") as mock_registry:
                mock_adapter = MagicMock()
                mock_adapter.update_workflow_tags = AsyncMock(return_value={
                    "id": workflow["n8n_workflow_id"],
                    "tags": [{"name": t} for t in new_tags],
                })
                mock_registry.get_adapter_for_environment.return_value = mock_adapter

                mock_db.update_workflow_tags = AsyncMock(return_value=None)

                response = client.put(
                    f"/api/v1/workflows/{workflow['n8n_workflow_id']}/tags",
                    params={"environment_id": "env-1"},
                    json={"tags": new_tags},
                    headers=auth_headers
                )
...
class TestWorkflowsAPIGitHub:
    """Tests for GitHub sync endpoints."""

    @pytest.mark.api
    def test_sync_to_github_success(self, client: TestClient, auth_headers):
        """POST /workflows/sync-to-github should sync workflows to GitHub."""
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            mock_db.get_environment = AsyncMock(return_value={
                "id": "env-1",
                "n8n_base_url": "https://n8n.example.com",
            ...
            with patch("app.api.endpoints.workflows.ProviderRegistry") as mock_registry:
                ...
                with patch("app.api.endpoints.workflows.GitHubService") as mock_github:
                    mock_github_instance = MagicMock()
                    mock_github_instance.is_configured.return_value = True
                    mock_github.return_value = mock_github_instance

                    response = client.post(
                        "/api/v1/workflows/sync-to-github",
                        params={"environment_id": "env-1"},
                        headers=auth_headers
                    )
    ...
    @pytest.mark.api
    def test_sync_to_github_not_configured(self, client: TestClient, auth_headers):
        """POST /workflows/sync-to-github without GitHub config should return error."""
        with patch("app.api.endpoints.workflows.db_service") as mock_db:
            mock_db.get_environment = AsyncMock(return_value={
                "id": "env-1",
                "n8n_base_url": "https://n8n.example.com",
            ...
            assert response.status_code == 400
            assert "github" in response.json()["detail"].lower() or "git" in response.json()["detail"].lower()
